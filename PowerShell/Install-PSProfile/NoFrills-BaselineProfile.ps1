...
function sed($file, $find, $replace) {
  ...
  #>
  if (-not $file -or -not $find -or -not $replace) {
    Write-Host "Missing arguments for sed. Showing help for `sed`..." -ForegroundColor Yellow
    Get-Help sed -Full
    return
  }
  ...
  try {
    ...
    $new = [regex]::Replace($content, $escapedFind, [string]$replace)
    Set-Content -Path $file -Value $new -ErrorAction Stop
    Write-Host "Replaced occurrences of '$find' with '$replace' in '$file'." -ForegroundColor Green
  }
  catch {
    Write-Host "Error performing replacement: $_" -ForegroundColor Red
    Write-Host "Showing help for sed:" -ForegroundColor Yellow
    Get-Help sed -Full
  }
}
...
if (-not (Get-Module -ListAvailable -Name Terminal-Icons)) {
  try {
    ...
    if ($debug) {
      Write-Host "$($PSStyle.Foreground.Red)Failed to install Terminal-Icons module. Error: $_"
    }
  }
}
else {
  try {
    ...
    if ($debug) {
      Write-Host "$($PSStyle.Foreground.Red)Failed to import Terminal-Icons module. Error: $_"
    }
  }
}
...
Write-Host "Run $($PSStyle.Foreground.Yellow)Show-Features$($PSStyle.Reset) to display the list of supported features."
